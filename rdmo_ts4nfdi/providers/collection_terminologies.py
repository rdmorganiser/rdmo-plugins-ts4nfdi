...
from rdmo_ts4nfdi.integrations.ts4nfdi.payload import extract_results as extract_default_results
from rdmo_ts4nfdi.integrations.ts4nfdi.payload import get_first_value
...
class TS4NFDICollectionTerminologiesProvider(TS4NFDIBaseProvider):
    ...
    def filter_results(self, results, search):
        ...
        return [result for result in results if self.terminology_matches(result, normalized_search)]
    ...
    def terminology_matches(self, result, normalized_search):
        ...
        return any(value and normalized_search in value.casefold() for value in values)
    ...
    def build_help_html(self, result, provider_config):
        collection_label = get_first_value(result, ('collection_label',)) or provider_config.get('collection_label')
        ontology_id = get_first_value(result, ('ontologyId', 'label'))
